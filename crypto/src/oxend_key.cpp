...
private_key_t oxendKeyFromHex(const std::string& private_key_hex) {
    if (!sispopmq::is_hex(private_key_hex) || private_key_hex.size() != KEY_LENGTH * 2)
    ...
    sispopmq::from_hex(private_key_hex.begin(), private_key_hex.end(), private_key.begin());
...
private_key_ed25519_t::from_hex(const std::string& sc_hex) {
    ...
    sispopmq::from_hex(sc_hex.begin(), sc_hex.end(), key.data.begin());
